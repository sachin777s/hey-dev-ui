...
const RightSidebar = () => {
  ...
  return (
    <div className='w-[25%] min-h-screen border-r-1 hidden lg:block sticky top-0'>

      {/* What's Happening */}
      <div className='mt-6 py-4 mx-4 px-4 border-[0.5px] rounded-2xl'>
      ...
      {/* Who to Follow */}
      <div className='mt-6 mx-4 p-4 rounded-2xl border-[0.5px]'>
        <h1 className='text-2xl font-semibold'>Who to follow</h1>
        <div className='mt-4'>
          {
            Array(3).fill(0).map((_, i) => {
              return (
                <div key={i} className='mt-2 flex items-start justify-between hover:bg-slate-400'>
                  <div className="flex items-center gap-2 cursor-pointer">
                    <Avatar src="https://i.pravatar.cc/150?u=a04258114e29026702d" />
                    <div className="flex flex-col items-start">
                      <span className="font-semibold">Sachin Kumar</span>
                      <span className="opacity-70">@sachin8976</span>
                    </div>
                  </div>
                  <Button radius='full' color='primary'>Follow</Button>
                </div>
              )
            })
          }
